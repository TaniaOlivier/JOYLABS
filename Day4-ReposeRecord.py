##Exercise 4 section 1

from statistics import mode

array = ['[1518-02-22 23:58] Guard #2543 begins shift',
...
for data in array:
	if 'Guard' in data:
		...
		if count_sumary==0:
			newList_sumary[0]=guard + '.' + str(sleep)+ '.' + str(wakeup)+ '.' + str(int(wakeup)-int(sleep))
		else:
			newList_sumary.append(count_sumary-1)
			newList_sumary[count_sumary]=guard + '.' + str(sleep)+ '.' + str(wakeup)+ '.' + str(int(wakeup)-int(sleep))
		
		count_sumary=count_sumary+1
		sleep=0
		wakeup=0


#print newList_sumary

#########################################################################################################
#Obtain max guard minutes sleep

newList_total = ['']
guard_index_add=0
guard_index_search=0
option=0

for data in newList_sumary:
	
	count=0
	option=0

	guard,sleep,wakeup,minutes=data.split(".")
		
	for guard_search in newList_total:
		if "." in str(guard_search): 
			a,b=str(guard_search).split(".")
			if guard == a:
				guard_index_search = count
				count=0
				option=2
				break
			else:
	...
	if option == 2:			
		newList_total[guard_index_search]=str(guard) + '.' + str(int(b)+int(minutes))
		option=0
	else:
		if guard != '':
			if guard_index_add==0:
				newList_total[guard_index_add]=str(guard) + '.'+ str(minutes)
			else:
				newList_total.append(guard_index_add)
				newList_total[guard_index_add]=str(guard) + '.'+ str(minutes)
		
			guard_index_add=guard_index_add+1
...
for data in newList_total:
	if "." in str(data): 
		a,b=str(data).split(".")
		for data_evaluate in newList_total:
			if "." in str(data_evaluate): 
				c,d=str(data_evaluate).split(".")
				if d>b and d>max_guard_minutes:
					max_guard=c
					max_guard_minutes=d

#print 'Guard '+str(max_guard)
#print 'Minutes ' + str(max_guard_minutes)

#########################################################################################################
#Obtain minute recurrent

newList_minutes = ['']
newList_minutes_general = ['']
count=0
count_general=0
...
for data in newList_sumary:
	...
	guard,sleep,wakeup,minutes=data.split(".")
	sleep_data=int(sleep)		
	wakeup_data=int(wakeup)		

	if guard == max_guard:
		#for sleep_begin in range(int(wakeup)):
		while sleep_data != (wakeup_data+1):
			
			if count == 0:
				newList_minutes[count]= sleep_data
			else:
				newList_minutes.append(count-1)
				newList_minutes[count]= sleep_data
	...
	while sleep_data != (wakeup_data+1):
		...
		if count_general == 0:
			newList_minutes_general[count_general]=float(guard +'.'+ str(sleep_data))
		else:
			newList_minutes_general.append(count-1)
			newList_minutes_general[count_general]=float(guard +'.'+ str(sleep_data))
		
		sleep_data=sleep_data+1
		count_general=count_general+1

#print newList_minutes

max_minute_number=mode(newList_minutes)

#print 'Specific minute: '+ str(max_minute_number)
print 'Answer 1: '+ str(int(max_minute_number)*int(max_guard))


max_frecuency=mode(newList_minutes_general)
a,b=str(max_frecuency).split(".")
max_frecuency_answer=int(a)*int(b)
print 'Answer 2: '+ str(max_frecuency_answer)