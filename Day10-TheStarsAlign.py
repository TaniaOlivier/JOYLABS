...
#Exercise 10 section 1

import matplotlib.pyplot as plt 
import numpy as np 

array=['position=<-10810,  43870> velocity=< 1, -4>',
...
#array=['	position=< 9,  1> velocity=< 0,  2>	',
...
for data in array:
	a,b,c=data.split("<")
	position_x,e=b.split(",")
	position_y,f=e.split(">")
	velocity_x,e=c.split(",")
	velocity_y,f=e.split(">")

	if count_sumary==0:
		newList_position_x[0]=int(position_x)
		newList_position_y[0]=int(position_y)
		newList_velocity_x[0]=int(velocity_x)
		newList_velocity_y[0]=int(velocity_y)

	else:
		newList_position_x.append(count_sumary)
		newList_position_y.append(count_sumary)
		newList_velocity_x.append(count_sumary)
		newList_velocity_y.append(count_sumary)
		newList_position_x[count_sumary]=int(position_x)
		newList_position_y[count_sumary]=int(position_y)
		newList_velocity_x[count_sumary]=int(velocity_x)
		newList_velocity_y[count_sumary]=int(velocity_y)
...
#NOTE: You can change the second variable where you want to start,  
# at first I ran since second number one and I saved each image to analize them until  
# I identify a message, in this case I set the second near the answer.
second=10931
...
for period in range(3):
    ...
    count=0
    for data in newList_position_x:
        x.append(count)
        y.append(count)
        x[count]=(float(newList_position_x[count]+(newList_velocity_x[count]*second))/10000)
        y[count]=(float(newList_position_y[count]+(newList_velocity_y[count]*second))/10000)
        count=count+1

    plt.plot(x, y,'x')
    plt.title('Second'+str(second))
    plt.pause(0.05)
    plt.savefig(str(second)+'.png')
    plt.cla()
    second=second+1
